...
echo "=> Load WS Data \r\n";
$data = json_decode(file_get_contents("C:/xampp/htdocs/wsmon/data.json"), true);
...
foreach($data as $ws){
	...
	if(!curl_errno($ch)){
		...
		$db->query("INSERT INTO de_ws (dbname, time, response)VALUES('".$dbname."', '".$time."', '".json_encode($response)."')");
	}
}
